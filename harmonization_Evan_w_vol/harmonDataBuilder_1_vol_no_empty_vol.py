...
def extractDataSet(path, filename):
  ...
  with open(os.path.join(path, filename), 'r') as f:
    ...
    return skip_count, genderList_, scannerList_, volumeList_, headline_, extractedDataDir_
...
def main():
  ...
  skip_count, genderList0, scannerList0, volumeList0, headline, extractedDataDir = \
    extractDataSet(args.sourcedir, args.filename)

  if skip_count > 0:
    print("%d patients are skipped due to missing volume info" % skip_count)
  ...
  for colname, datalist in extractedDataDir.items():
    ...
    print("scannerList_ len: %d... gen len:%d... volume len:%d" % (len(scannerList0), len(genderList0), len(volumeList0)))
