...
def proc_dataset_file(filepath):
  ...
  with open(filepath) as f:
    ...
    # headline should be ['Dataset', 'subjectID', 'Age', 'Sex', 'Scanner type', 'Magnetic field of strength', 'Volume']
    # headline[-1] is the last column in DataSet, and it contains "\n" so need strip to remove the newline
    headline = ['Dataset'] + headline[2:5] + headline[8:10] + [headline[-1].strip()]
    print("Evan!!!!!!!")
    print(headline)
    print("Evan@@@@@@@@@@@@@@")
    ...
    for line in lines[1:]:
      lineList = line.split(',')
      # usefulData are [dataset, subjectID, Age, Sex, Scanner-type, magnetic field strength, Volume]
      usefulData = [lineList[0]] + lineList[2:5] + lineList[8:10] + [lineList[-1].strip()]
...
def main():
  ...
  outputheadline = ["Dataset", "subjectId", "Age", "Sex", "Scanner type", "Magnetic field of strength", "Volume"]
