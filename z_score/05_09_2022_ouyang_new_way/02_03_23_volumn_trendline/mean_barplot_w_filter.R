...
if (is_google) {
  ...
  df0 <- read.csv("lh_rh_lobes_age_20230323_long_w_dataset.csv")
  #df0 <- read.csv("eTIV_raw_all_w_rockland.csv")
}

if (filter_data) {
  print(paste("Number of rows before filtering:", nrow(df0)))
  df0 <- df0[ which(df0$Age!="" & df0$Age!="Siemens"
                    # & (df0$Sex==1 | df0$Sex==2 |
                    # df0$Sex=="M" | df0$Sex=="m" | df0$Sex=="F" |
                    # df0$Sex=="f")
                    # & df0$Dataset!="ABCD"
                    #& df0$Dataset!="NIH_PD"
  ),  ]
  print(paste("Number of rows after filtering:", nrow(df0)))
}
