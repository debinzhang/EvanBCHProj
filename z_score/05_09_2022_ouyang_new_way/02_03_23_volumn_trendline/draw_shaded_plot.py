...
def draw_shaded_plot(feature):
  plot_item_list = [
      {"file_type": "_raw_no_all_plot", "plot_title":"raw Data with outlier removed"},
      {"file_type": "_harmo_no_all_plot", "plot_title":"harmonized Data with outlier removed"}
  ]
  for plot_item in plot_item_list:
    src_file_path = feature + plot_item["file_type"] + ".csv"
    if not os.path.isfile(src_file_path):
      print("cannot find file: %s, skip drawing lineplot" % src_file_path)
      continue
    output_file_path = feature + plot_item["file_type"] + "_lineplot.png"
    print("draw_shaded_plol(): working on %s" % src_file_path)
    df = pd.read_csv(src_file_path)

    fig, ax = plt.subplots(figsize=a4_dims)
    sns_lineplot = sns.lineplot(ax=ax, x='Age', y=feature, data=df.query("Sex != 'Unknown'"), hue='Sex')
    plt.title(feature + " " + plot_item["plot_title"])
    fig_lineplot = sns_lineplot.get_figure()
    fig_lineplot.savefig(output_file_path)
    plt.clf()
    plt.close()
...
def draw_shaded_plot_smooth(feature):
  ...
  #fig, ax = plt.subplots(figsize=a4_dims)
  sns_lineplot = sns.lineplot(x='Age', y=feature, data=df_final.query("Sex != 'Unknown'"), hue='Sex')
  plt.show()
