...
getRobustZScoreByDataset <- function(data, feature) {
  ...
  # if (byETIV) {
  #   grouped_data <- data %>% group_by(Dataset) %>% summarise_at(vars(eTIV), list(group_median = median, group_mad = mad))
  #   merged_data <- merge(data, grouped_data, by="Dataset")
  #   zscore_data<-merged_data %>% mutate(zscore=(eTIV - group_median)/group_mad)
  # } else {
  #   # this is by volume case
  #   grouped_data <- data %>% group_by(Dataset) %>% summarise_at(vars(Volume), list(group_median = median, group_mad = mad))
  #   merged_data <- merge(data, grouped_data, by="Dataset")
  #   zscore_data<-merged_data %>% mutate(zscore=(Volume - group_median)/group_mad)
  # }
  return(zscore_data)
}
...
gen_raw_harmo_data <- function() {
  ...
  for (file_path in file_list) {
    ...
    data_raw_1 <- subset(data_raw_1, select=-c(path, subjectId, Volume))
...
process_lh_curvind <- function() {
  lh_curvind_region_list0 <- list("BrainSegVolNotVent", "eTIV",
                                 "lh_bankssts_curvind",
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3,
                          region_list=lh_curvind_region_list, outfile="lh_curvind_anova_result.xlsx", 
...
process_rh_curvind <- function(draw_hemi_plot) {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3,
                          region_list=rh_curvind_region_list, outfile="rh_curvind_anova_result.xlsx",
...
process_lh_foldind <- function() {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3,
                          region_list=lh_foldind_region_list, outfile="lh_foldind_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_rh_foldind <- function(draw_hemi_plot) {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3,
                          region_list=rh_foldind_region_list, outfile="rh_foldind_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_lh_meancurv <- function() {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3,
                          region_list=lh_meancurv_region_list, outfile="lh_meancurv_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_rh_meancurv <- function(draw_hemi_plot) {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3, 
                          region_list=rh_meancurv_region_list, outfile="rh_meancurv_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_lh_gauscurv <- function() {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3, 
                          region_list=lh_gauscurv_region_list, outfile="lh_gauscurv_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_rh_gauscurv <- function(draw_hemi_plot) {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3, 
                          region_list=rh_gauscurv_region_list, outfile="rh_gauscurv_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_lh_thicknessstd <- function() {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3,
                          region_list=lh_thicknessstd_region_list, outfile="lh_thicknessstd_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
...
process_rh_thicknessstd <- function(draw_hemi_plot) {
  ...
  gen_4_sheet_all_regions(data_raw, data_harmo, sex=3, 
                          region_list=rh_thicknessstd_region_list, outfile="rh_thicknessstd_anova_result.xlsx",
                          genTukeyHSD=TRUE, genShadedPlot=TRUE)
