...
def getSubjIdDatasetFromPath(path):
  ...
  if "/ABIDE_I/" in path:
    ...
    result = re.search('(.*)/BCH/3NIFTI_FreeSurfer/([0-9]*)-(.*)', path)
    if result is not None:
      # subjId = 4574338
      subjId = result.group(2)
    else:
    ...
    dataset = "ABCD"
    # path = "/neuro/labs/grantlab/research/MRI_Predict_Age/ABCD/3NIFTI_FreeSurfer/NDAR_INV00BD7VDC"
    # result = re.search('(.*)ABCD/3NIFTI_FreeSurfer/(.*)')
    # subjId = 'NDAR_INV00BD7VDC'
    # subjId = result.group(2)
    result = re.search('(.*)/ABCD/3NIFTI_FreeSurfer/(.*)', path)
    if result is not None:
      subjId = result.group(2)
    else:
      print("xxxx:%s" % path)
  ...
  elif "/NKI_Rockland/" in path:
    dataset = "NKI_Rockland"
    # path = "/neuro/labs/grantlab/research/MRI_Predict_Age/NKI_Rockland/3NIFTI_FreeSurfer/3366302"
    # result = re.search('(.*)NKI_Rockland/3NIFTI_FreeSurfer/(.*)')
    # subjId = 3366302
    # subjId = result.group(2)
    result = re.search('(.*)NKI_Rockland/3NIFTI_FreeSurfer/(.*)', path)
    if result is not None:
      subjId = result.group(2)
    else:
      failToParse = True
...
def main():
  ...
  miss_subjId_count = 0
  # rockland data does not contain scanner type and megnetic Strength. So skip them for now
  ignore_rockland_count = 0
  ...
  with open(args.sourcefile, 'r') as srcf:
    ...
    for line in lines[1:]:
      ...
      if dataset == 'NKI_Rockland':
        ignore_rockland_count +=1
        continue
    ...
    if ignore_rockland_count>0:
      print("ignore %d NKI_Rockland records because they do not contain scanner type and strength info." % ignore_rockland_count)
