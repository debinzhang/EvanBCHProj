...
#file_list <-list("all_subjects_cortical_metrics_LH_thickness_09_15_2022_preHarmo.csv")
# file_list <-list("all_subjects_cortical_metrics_LH_curvind_09_15_2022_preHarmo.csv")

for (file_path in file_list) {
    #data_raw <- read.csv("test.csv", stringsAsFactors = TRUE)
    ...
    # print(paste("raw data row #: ", row_num_before, sep = ""))
    
    #data_raw_1 <- subset(data_raw, data_raw$Sex != "")
    data_raw_1 <- data_raw[ which(data_raw$Age!="" & data_raw$Age!="Siemens"  &
                   ...
                   # & data_raw$Dataset!="BCH"
                   ),  ]
    row_num_after_sex <- nrow(data_raw_1)
    # print(paste("after check sex #: ", row_num_after, " ... ", (row_num_before-row_num_after), 
    #             " rows removed",  sep = ""))
    ...
    print("------------")
    #harmonize(outfile, harmo_file, scanertype=FALSE, Eb=FALSE, Mean_only=TRUE, dataset=FALSE, Age=TRUE)
