...
def harmonize(srcfile, output, scanertype=False, dataset=False, Eb=True, Mean_only=False, Parametric=True, Age=False, batch_key=1):
  ...
  if (batch_key==1):
    ...
    batch_col='scanertype'
    # if (not 'batch' in categorical_cols):
    #   categorical_cols.append('batch')
    if ('scanertype' in categorical_cols):
      categorical_cols.remove('scanertype')
...
#harmonize("all_subjects_cortical_metrics_LH_thickness_09_15_2022_preHarmo_clean.csv", "test.csv")
