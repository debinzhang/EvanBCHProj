...
data_analyse <- function(data_raw, data_harmo, feature, gen_csv=TRUE, gen_TukeyHSD=FALSE, 
  ...
  #rslt <- harmonize(src_no_file, outfile, scanertype=FALSE, Eb=FALSE, Mean_only=TRUE, dataset=TRUE)
  rslt <- harmonize(src_no_file, outfile, scanertype=FALSE, Eb=FALSE, Mean_only=TRUE, dataset=FALSE, Age=TRUE)
   if (!rslt) {
...
gen_4_sheet_all_regions(data_raw, data_harmo, sex=3, byMeanThick=TRUE, 
                        region_list=lh_thickness_region_list, outfile="lh_thickness_anova_all_age_as_key.xlsx", IsLH=TRUE, genTukeyHSD=TRUE)
