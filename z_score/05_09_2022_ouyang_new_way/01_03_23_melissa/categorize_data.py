...
def categorize_data(clean_leftover=False):
  ...
  files = [f for f in glob.glob("*.png") if  ("_harmo_no_all.png" in f or "_harmo_no_all_s.png" in f) ]
  for f in files:
  ...
  files = [f for f in glob.glob("*.xlsx") if ("anova_age_as_key.xlsx" in f) ]
  for f in files:
  ...
  if(clean_leftover):
    ...
    files = [f for f in glob.glob("*_all.csv") + glob.glob("*_all_plot.csv") ]
    for f in files:
      os.remove(f)
