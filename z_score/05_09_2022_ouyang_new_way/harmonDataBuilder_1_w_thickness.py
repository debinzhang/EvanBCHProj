...
def extractDataSet(path, filename):
  ...
  with open(os.path.join(path, filename), 'r') as f:
    ...
    # generate thicknesslist
    thicknessList_ = list(extractedDataDir_['rh_MeanThickness_thickness'])

    return genderList_, scannerList_, thicknessList_, headline_, extractedDataDir_
...
def main():
  ...
  genderList0, scannerList0, thicknessList0, headline, extractedDataDir = \
  ...
  with open(args.output, "w") as f:
    ...
    f.write("#,Age,Dataset,origin_thickness,%s\n" % (','.join(colnamelist)))
    for i in range(len(genderList0)):
      ...
      f.write("%d,%s,%s,%s,%s\n" % ((i+1),\
       extractedDataDir['Age'][i], extractedDataDir['Dataset'][i], extractedDataDir['rh_MeanThickness_thickness'][i], \
